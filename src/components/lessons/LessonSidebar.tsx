...
interface LessonSidebarProps {
    ...
    lessonProgress?: any[];
    courseTitle?: string;
    quizzesByModule?: Record<string, any>;
}
                            ...
                            {expandedModules.has(module.id) && (
                                <div className="bg-white">
                                    {(() => {
                                        ...
                                        return combined.map((item: any) => {
                                            if (item._type === 'lesson') {
                                                ...
                                                const chapter = item;
                                                return (
                                                    <div key={chapter.id} className="bg-gray-50/30">
                                                        <div className="px-6 py-2 bg-gray-100/50 flex items-center gap-2">
                                                            <ChevronRight className="w-3 h-3 text-gray-400" />
                                                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tight">{chapter.title}</span>
                                                        </div>
                                                        {chapter.lessons?.sort((a: any, b: any) => a.order_index - b.order_index).map((lesson: any) => {
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    onClick={() => navigate(`/courses/${courseId}/lessons/${lesson.id}`)}
                                                                    className={cn(
                                                                        "w-full flex items-center gap-3 pl-8 pr-6 py-3 text-sm transition-all border-l-4",
                                                                        isActive
                                                                            ? "bg-blue-50 border-blue-600 text-blue-700 font-semibold"
                                                                            : "bg-white border-transparent text-gray-600 hover:bg-gray-50"
                                                                    )}
                                    ...
                                    {quizzesByModule?.[module.id] && (
                                        <button
                                            onClick={() => navigate(`/courses/${courseId}`, { state: { openModuleQuiz: module.id } })}
                                            className={cn(
                                                "w-full flex items-center gap-3 px-6 py-4 text-sm transition-all border-l-4 bg-primary/5 text-primary border-transparent hover:bg-primary/10"
                                            )}
                                        >
                                            <div className="shrink-0">
                                                <CheckCircle className="w-4 h-4" />
                                            </div>
                                            <span className="flex-1 text-left font-semibold">{quizzesByModule[module.id].title}</span>
                                        </button>
                                    )}
                                </div>
                            )}
