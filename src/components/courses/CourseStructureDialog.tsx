...
const CourseStructureDialog = ({ courseId, courseTitle, open, onOpenChange, userRole }: CourseStructureDialogProps) => {
    ...
    const { data: structureData, refetch: refetchStructure } = useQuery({
        ...
        queryFn: async () => {
            ...
            // Get final assessment for course using RPC to bypass cache issues
            const { data: finalQuizData, error: rpcError } = await supabase
                .rpc("get_final_assessment", { p_course_id: courseId })
                .maybeSingle();

            if (finalQuizData) {
                // Fetch questions separately since RPC returns only quiz rows
                const { data: questions } = await supabase
                    .from("quiz_questions")
                    .select("*")
                    .eq("quiz_id", finalQuizData.id)
                    .order("order_index");

                finalAssessment = { ...finalQuizData, quiz_questions: questions || [] };
            }

            return { modules, quizzesByLesson, quizzesByModule, finalAssessment };
        },
        enabled: !!courseId && open,
    });

    // Update form order index when structure loads
    const [hasInitializedOrder, setHasInitializedOrder] = useState(false);
    if (structureData?.modules && !hasInitializedOrder) {
        const nextOrder = structureData.modules.length > 0
            ? Math.max(...structureData.modules.map((m: any) => m.order_index)) + 1
            : 1;
        setModuleForm(prev => ({ ...prev, order_index: nextOrder }));
    ...
    const createModuleMutation = useMutation({
        mutationFn: async () => {
            ...
            // Fetch current max order to allow concurrent safe additions
            ...
            const { error } = await supabase
                ...
                .insert({
                    ...
                    order_index: nextOrder, // Force logical next order
                });
            if (error) throw error;
        },
        onSuccess: async () => {
            const { data: updatedData } = await refetchStructure();
            // Calculate next order index using FRESH data
            const nextOrder = updatedData?.modules && updatedData.modules.length > 0
                ? Math.max(...updatedData.modules.map((m: any) => m.order_index)) + 1
                : 1;
            setModuleForm({ title: "", description: "", order_index: nextOrder });
    ...
    const [finalAssessmentForm, setFinalAssessmentForm] = useState({ title: "Évaluation Finale", passing_score: 80, time_limit: 60 });
    ...
    const createFinalAssessmentMutation = useMutation({
        mutationFn: async () => {
            if (!courseId) throw new Error("Cours non sélectionné");

            // Use RPC function to create final assessment
            const { data, error } = await supabase
                .rpc("create_final_assessment_rpc", {
                    p_course_id: courseId,
                    p_title: finalAssessmentForm.title,
                    p_passing_score: finalAssessmentForm.passing_score,
                    p_time_limit: finalAssessmentForm.time_limit,
                });

            if (error) throw error;
            if (!data?.success) throw new Error("Échec de la création de l'évaluation finale");
        },
        onSuccess: async () => {
            await refetchStructure();
            toast.success("Évaluation finale créée avec succès! 🎉");
        },
        onError: (e: any) => {
                    ...
                    <div className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <Input placeholder="Ordre (Auto)" type="number" value={moduleForm.order_index} disabled className="bg-muted text-muted-foreground cursor-not-allowed" />
                            </div>
                            <div className="flex justify-end">
                                <Button onClick={() => createModuleMutation.mutate()} disabled={!moduleForm.title || createModuleMutation.isPending}>
                                    {createModuleMutation.isPending ? "Ajout..." : "Ajouter le module"}
                                </Button>
                            </div>
                            ...
                            {/* Final Assessment Section - Moved to top level */}
                            <div className="border rounded-lg p-4 bg-yellow-50/30 border-yellow-100">
                                <h3 className="font-semibold text-sm flex items-center gap-2 mb-4">
                                    <Award className="w-5 h-5 text-yellow-600" />
                                    Configuration de l'Évaluation Finale
                                </h3>

                                {structureData?.finalAssessment ? (
                                    <div className="p-4 border rounded-lg bg-white flex items-center justify-between shadow-sm">
                                        <div>
                                            <div className="font-bold flex items-center gap-2 text-lg">
                                                {structureData.finalAssessment.title}
                                                <Badge className="bg-yellow-600 hover:bg-yellow-700">Final</Badge>
                                            </div>
                                            <div className="text-sm text-muted-foreground mt-1">
                                                Score min: {structureData.finalAssessment.passing_score}% • Durée: {structureData.finalAssessment.time_limit_minutes || 60} min
                                            </div>
                                        </div>
                                        <div className="flex gap-2">
                                            <Button variant="outline" onClick={() => {
                                                setEditingQuizId(structureData.finalAssessment.id);
                                                setIsQuestionDialogOpen(true);
                                            }}>
                                                Gérer les questions ({structureData.finalAssessment.quiz_questions?.length || 0})
                                            </Button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="bg-white p-4 rounded-lg border shadow-sm">
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                            <div className="md:col-span-1 space-y-1">
                                                ...
                                                <Input
                                                    value={finalAssessmentForm.title}
                                                    onChange={(e) => setFinalAssessmentForm({ ...finalAssessmentForm, title: e.target.value })}
                                                    className="bg-white"
                                                />
                                            </div>
                                            <div className="md:col-span-1 space-y-1">
                                                <Label className="text-xs font-medium">Score passager (%)</Label>
                                                <Input
                                                    type="number"
                                                    value={finalAssessmentForm.passing_score}
                                                    onChange={(e) => setFinalAssessmentForm({ ...finalAssessmentForm, passing_score: parseInt(e.target.value || "80") })}
                                                    className="bg-white"
                                                />
                                            </div>
                                            <div className="md:col-span-1 space-y-1">
                                                <Label className="text-xs font-medium">Durée limite (min)</Label>
                                                <Input
                                                    type="number"
                                                    value={finalAssessmentForm.time_limit}
                                                    onChange={(e) => setFinalAssessmentForm({ ...finalAssessmentForm, time_limit: parseInt(e.target.value || "60") })}
                                                    className="bg-white"
                                                />
                                            </div>
                                            <Button
                                                className="w-full bg-yellow-600 hover:bg-yellow-700 text-white shadow-sm"
                                                onClick={() => createFinalAssessmentMutation.mutate()}
                                            >
            ...
            <Dialog open={isLessonDialogOpen} onOpenChange={setIsLessonDialogOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Modifier les détails et le contenu de la leçon.
                        </DialogDescription>
                    </DialogHeader>
                    {selectedLesson && (
                        <div className="space-y-4">
                ...
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    {(() => {
                        ...
                        const allQuizzes = [
                            ...
                            ...(structureData?.finalAssessment ? [structureData.finalAssessment] : [])
                        ];
